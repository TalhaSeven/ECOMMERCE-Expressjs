  ...
  async pay(req: Request, res: Response) {
    ...
    if (campaign) {
      ...
      campaignPrice = Number(
        list.reduce((acc: number, o: any) => acc + parseFloat(o.price), 0)
      );
    }
    ...
    if (list.length > 0) {
      ...
      const total =
        basketList.reduce((acc, o) => acc + parseFloat(o.total), 0) -
        (discountCouponPrice + campaignPrice);
      const totalTax = basketList.reduce(
        (acc, o) => acc + parseFloat(o.tax),
        0
      );
      ...
      await axios
        .post("http://localhost:3070/validation", {
          cardName: cardName,
      ...
      if (provisionCode) {
        await axios
          .post("http://localhost:3070/payment", {
            code: provisionCode,
            price: total,
          })
          .then(async (response: any) => {
            ...
            const payId = await movementRepository.payHeaderInsert(
              userId,
              total,
              totalTax,
              discountCouponPrice + campaignPrice
  ...
  async test(req: Request, res: Response) {
    await axios
      ...
      .then((response: any) => {
        ...
        return res
          .status(200)
          .send({ message: "success", data: response.data });
      })
      .catch((error) => {
        console.log(error);
